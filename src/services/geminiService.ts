...
/**
 * Analyzes an image for recycling information.
 * @param imageBase64 The base64-encoded string of the image.
 * @returns A JSON string with the analysis.
 */
export async function analyzeRecyclingImage(imageBase64: string) {
  try {
    const request = {
      contents: [
        {
    ...
    console.error("ERROR IN GOOGLE SERVICE:", error);
